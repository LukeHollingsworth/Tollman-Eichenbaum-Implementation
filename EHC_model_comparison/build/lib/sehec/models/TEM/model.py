...
class TEM(NeuralResponseModel):
    def __init__(self, model_name="TEM", **mod_kwargs):
        ...
        self.poss_objects, self.objects = None, None
    ...
    def update(self, obs, d, it_num, seq_index, it_index):
        ...
        states = np.zeros(shape=(self.pars['batch_size'], self.pars['t_episode'] + 1))
        ...
        for batch in range(self.pars['batch_size']):
            if seq_index == 0:
                ...
                self.start_states = states[batch, int(self.pars['t_episode'])]
            else:
                start = self.start_states[batch]
                x[batch, :, 0] = self.objects[batch][int(start)]
                for step in range(1, self.pars['t_episode']):
                    state = self.obs_to_states(obs[batch, :, step], batch)
                    observation = self.objects[batch][state]
                    states[batch, step] = state
                    x[batch, :, step] = observation
        ...
        self.gs, self.x_ = self.save_and_plot(x, states, it_num, gs, ps, ps_gen, x_gt, x_s, a_rnn, a_rnn_inv, acc_gt)

        if seq_index == self.pars['n_episode'] - 1 and it_index == self.pars['train_iters'] - 1:
