...
def single_plot(cell, width, choice, maxmin=False, shiny=(), hexy='no', lims=False, smoothing=False, mult=2, cmap='jet',
                mask=False, n_laps=4):
    cell_ = cell[:, choice]
    ...
    if lims:
        plt.imshow(cell_reshaped, cmap=cmap, interpolation=interpolation_method, vmin=lims[0][choice],
                vmax=lims[1][choice])
    else:
        plt.imshow(cell_reshaped, cmap=cmap, interpolation=interpolation_method)
    ...
    if hexy == 'loop_laps':
        ...
        for iii in range(n_laps):
            ...
            plt.plot([n_states * iii / n_laps - 1, n_states * iii / n_laps - 1], [-1, 14], linewidth=2,
                    color='w')
...
def get_path(run, date, save_dirs, recent=-1):
    for save_dir in save_dirs:
        try:
            save_path = save_dir + date + '/normal_run' + str(run) + '/save'
...
def get_data(save_dirs, run, date, recent=-1):
    ...
    data = (a_rnn, x_all, g_all, p_all, p_gen_all, acc_s_t_to, acc_s_t_from, positions, shinys, adj, timeseries)
...
def sort_data(g_all, p_all, shinys, widths, mult, smoothing, params, batch_id, g_max_0=False, p_max_0=True):
    ...
    return shinys, masks, g_lim, p_lim
